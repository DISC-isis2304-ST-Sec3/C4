...
public class ControladorHabitaciones {
 @Autowired
  ...
  public String createHabitacion(
    ...
    Hoteles hoteles = hotelesRepositorio.obtenerHoteles(idhotel);

    habitacionesRepositorio.crearHabitacion(tipo, capacidad, costo, hoteles.getId());
    return "redirect:/habitaciones";
  }

  @GetMapping("/{id}/edit")
  public String editHabitacionForm(@PathVariable(name = "idhabitacion") Long idhabitacion, Model model) {
    List<Hoteles> hoteles = new ArrayList<>(hotelesRepositorio.obtenerHoteles().stream().toList());
    Habitacion habitacion = habitacionesRepositorio.obtenerHabitacion(idhabitacion);
    hoteles.add(habitacion.getHoteles());
  ...
  public String actualizarHabitacion(
      @PathVariable(name = "idhabitacion") Long idhabitacion,
      @ModelAttribute(name = "tipo") String tipo,
      @ModelAttribute(name = "capacidad") Integer capacidad,
      @ModelAttribute(name = "costo") Integer costo,
      @ModelAttribute(name = "idhotel") long idhotel
    ...
    habitacionesRepositorio.actualizarHabitacion(
        idhabitacion, tipo, capacidad, costo, hoteles.getId());
    return "redirect:/habitaciones";
  }

  @GetMapping("/{id}/delete")
  public String eliminarHabitacion(@PathVariable(name = "idhabitacion") Long idhabitacion) {
